...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'unboundfate', 'UnboundFate', and 'UNBOUNDFATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.unboundfate/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.unboundfate/g),
   ...
   * Rename files.
   *
   * Rename files that had unboundfate in their name, such as
   * css/unboundfate.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*unboundfate*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('unboundfate', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-unboundfate-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob UnboundFate's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace unboundfate name mentions in files.
      generator.replaceFileContents();
      // Rename files that had unboundfate in their name.
